<template>
  <div class="tool-panel">
    <div class="session-sidebar">
      ...
      <div v-for="s in sessions" :key="s.id" :class="['session-item', {active: activeId===s.id}]" @click="$emit('update:activeId', s.id)" @contextmenu.prevent="$emit('show-context-menu', $event, 'tcpServer', s)">
      ...
      <div class="sm-left" v-show="showLeftPanel">
        <div class="card">
          ...
          <div style="display:flex;gap:8px;">
            <div style="flex:1"><label style="font-size:11px;color:var(--text-secondary);">{{ $t('port') }}</label><el-input-number v-model="activeSession.port" :min="1" :max="65535" size="small" style="width:100%;margin-top:2px;"></el-input-number></div>
            <div style="flex:1"><label style="font-size:11px;color:var(--text-secondary);">{{ $t('maxConn') }}</label><el-input-number v-model="activeSession.maxConn" :min="1" :max="100" size="small" style="width:100%;margin-top:2px;"></el-input-number></div>
          </div>
          <div style="margin-top:10px;display:flex;gap:6px;">
            <el-button v-if="!activeSession.running" class="btn-start" size="small" @click="startServer(activeSession)" :loading="activeSession.loading" style="flex:1;">{{ $t('start') }}</el-button>
            <el-button v-if="activeSession.running" class="btn-stop" size="small" @click="stopServer(activeSession)" style="flex:1;">{{ $t('stop') }}</el-button>
          </div>
          <div class="stats-bar">
            <span class="stat-item">{{ $t('status') }}：<span class="stat-value" :style="{color:activeSession.running?'var(--accent-green)':'var(--text-secondary)'}">{{ activeSession.running ? $t('running') : $t('stopped') }}</span></span>
            <span class="stat-item">{{ $t('clients') }}：<span class="stat-value">{{ activeSession.clients.length }}</span></span>
          </div>
        </div>
        <div class="card" v-if="activeSession.running && activeSession.clients.length>0">
          <div class="card-title">{{ $t('connectedClients') }}</div>
          <div v-for="c in activeSession.clients" :key="c" class="client-item">
            ...
            <div style="display:flex;gap:4px;">
              ...
              <el-button size="mini" type="text" @click="disconnectClient(activeSession,c)" style="color:var(--accent-red);padding:0 4px;">{{ $t('kick') }}</el-button>
            </div>
          </div>
        </div>
        <div class="card send-card" v-if="activeSession.running">
          ...
          <div v-else style="margin-bottom:6px;"><el-tag size="mini" type="warning">{{ $t('broadcastAll') }}</el-tag></div>
          ...
          <div style="margin-top:6px;display:flex;gap:6px;">
            ...
            <el-select v-model="activeSession.encoding" size="small" style="width:100px;" :disabled="activeSession.format === 'hex'"><el-option label="UTF-8" value="UTF-8"></el-option><el-option label="GBK" value="GBK"></el-option><el-option label="ASCII" value="ASCII"></el-option></el-select>
    ...
    <div class="empty-state" v-else>{{ $t('emptyTcpServer') }}</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    startServer(s) { s.loading = true; callJava('startTcpServer', s.id, s.port, s.maxConn) },
    stopServer(s) { callJava('stopTcpServer', s.id); s.running = false; s.clients = [] },
    sendMessage(s) {
      if (!s.message.trim()) return
      var m = s.message.replace(/\\n/g, '\n').replace(/\\r\\n/g, '\r\n')
      if (s.targetClient) callJava('tcpServerSend', s.id, s.targetClient, m, s.encoding, s.format)
      else callJava('tcpServerSendAll', s.id, m, s.encoding, s.format)
      s.message = ''
    },
    disconnectClient(s, cid) {
      var idx = s.clients.indexOf(cid)
      if (idx >= 0) s.clients.splice(idx, 1)
      callJava('tcpServerDisconnectClient', s.id, cid)
    },
    clearLogs(session) {
      if (!session) return
      session.logs = []
