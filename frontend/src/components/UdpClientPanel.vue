<template>
  <div class="tool-panel">
    <div class="session-sidebar">
      ...
      <div v-for="s in sessions" :key="s.id" :class="['session-item', {active: activeId===s.id}]" @click="$emit('update:activeId', s.id)" @contextmenu.prevent="$emit('show-context-menu', $event, 'udpClient', s)">
        <span class="s-name">{{ s.id }}</span>
        <span :class="['s-status', s.bound?'on':'off']">{{ s.bound ? $t('bound') : $t('unbound') }}</span>
    ...
    <div class="session-main" v-if="activeSession">
      ...
      <div class="sm-left" v-show="showLeftPanel">
        <div class="card">
          <div class="card-title">{{ $t('udpClient') }} · {{ activeSession.id }}</div>
          <div style="margin-bottom:10px;"><label style="font-size:11px;color:var(--text-secondary);">{{ $t('localPort') }}</label><el-input-number v-model="activeSession.port" :min="1" :max="65535" size="small" style="width:100%;margin-top:2px;"></el-input-number></div>
          <el-button v-if="!activeSession.bound" class="btn-start" size="small" @click="bind(activeSession)" :loading="activeSession.loading" style="width:100%;">{{ $t('bind') }}</el-button>
          <el-button v-if="activeSession.bound" class="btn-stop" size="small" @click="unbind(activeSession)" style="width:100%;">{{ $t('unbind') }}</el-button>
          <div class="stats-bar"><span class="stat-item">{{ $t('status') }}：<span class="stat-value" :style="{color:activeSession.bound?'var(--accent-green)':'var(--text-secondary)'}">{{ activeSession.bound ? $t('bound') : $t('unbound') }}</span></span></div>
        </div>
        <div class="card send-card" v-if="activeSession.bound">
          <div class="card-title">{{ $t('sendDatagram') }}</div>
          <div style="display:flex;gap:8px;margin-bottom:8px;">
            <div style="flex:1"><label style="font-size:11px;color:var(--text-secondary);">{{ $t('targetHost') }}</label><el-input v-model="activeSession.sendHost" size="small" placeholder="127.0.0.1" style="margin-top:2px;"></el-input></div>
            <div style="flex:0 0 110px"><label style="font-size:11px;color:var(--text-secondary);">{{ $t('targetPort') }}</label><el-input-number v-model="activeSession.sendPort" :min="1" :max="65535" size="small" style="width:100%;margin-top:2px;"></el-input-number></div>
          </div>
          <el-input type="textarea" v-model="activeSession.message" :placeholder="$t('inputPlaceholderUdp')"></el-input>
          <div style="margin-top:6px;display:flex;gap:6px;">
            ...
            <el-button class="btn-send" size="small" @click="sendMessage(activeSession)" style="flex:1;">{{ $t('send') }}</el-button>
          </div>
        </div>
      </div>
      <div class="sm-right">
    ...
    <div class="empty-state" v-else>{{ $t('emptyUdpClient') }}</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    bind(s) { s.loading = true; callJava('startUdpClient', s.id, s.port) },
    unbind(s) { callJava('stopUdpClient', s.id); s.bound = false },
    sendMessage(s) {
      if (!s.message.trim()) return
      callJava('udpClientSend', s.id, s.sendHost, s.sendPort, s.message, s.encoding, s.format)
      s.message = ''
    },
    clearLogs(session) {
      if (!session) return
